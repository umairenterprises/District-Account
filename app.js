...
function fillDonorSelect() {
  const sel = document.getElementById("f_donor");
  const current = sel.value;
  const list = (dropdowns.donor || []).slice().sort((a, b) => a.localeCompare(b));
  sel.innerHTML = '<option value="" disabled selected>Donor select karein…</option>' +
    list.map((d) => `<option value="${escapeHtml(d)}">${escapeHtml(d)}</option>`).join("") +
    '<option value="__new__">+ Naya Donor Add Karein</option>';
  if (list.includes(current)) sel.value = current;
  toggleDonorNewInput();
}

function toggleDonorNewInput() {
  const sel = document.getElementById("f_donor");
  const newInput = document.getElementById("f_donor_new");
...
function getDonorValue() {
  const sel = document.getElementById("f_donor");
  if (sel.value === "__new__") return document.getElementById("f_donor_new").value.trim();
  return sel.value;
}

function maybeAddNewDonor(name) {
  if (!name) return;
  dropdowns.donor = dropdowns.donor || [];
  if (!dropdowns.donor.includes(name)) {
    dropdowns.donor.push(name);
    saveDropdowns();
  }
}
...
function bindForm() {
  ...
  document.getElementById("f_donor").addEventListener("change", toggleDonorNewInput);
...
function checkDuplicate() {
  ...
  if (!book || !receipt) { hideDup(); saveBtn.disabled = false; return; }
  const dup = entries.find((e) => e.book === book && e.receipt === receipt && e.id !== editingId);
  const warnEl = document.getElementById("dupWarning");
  warnEl.classList.toggle("hidden", !dup);
  if (dup) {
    warnEl.textContent = `⚠ Book No ${book} + Receipt No ${receipt} pehle se maujood hai (${dup.donor || "—"}) — pehle wo change karein`;
    warnEl.classList.add("strict");
...
function saveEntry() {
  ...
  const dup = entries.find((e) => e.book === book && e.receipt === receipt && e.id !== editingId);
  if (dup) {
    showToast("⚠ Duplicate Book + Receipt No — save nahi hui, pehle change karein");
    checkDuplicate();
    return;
  }

  const donorValue = getDonorValue();
  if (!donorValue) {
    showToast("Donor select ya naya naam likhein");
    return;
  }
  ...
  if (editingId) {
    wsRef().collection("entries").doc(editingId).update(data)
      .then(() => { showToast("Entry update ho gayi"); maybeAddNewDonor(donorValue); resetForm(); })
    ...
    wsRef().collection("entries").add(data)
      .then(() => { showToast("Receipt record save ho gayi"); maybeAddNewDonor(donorValue); resetForm(); })
      .catch((e) => showToast("Error: " + e.message));
  }
}
...
function resetForm() {
  ...
  document.getElementById("saveBtn").textContent = "Receipt Record Save Karein";
...
function editEntry(id) {
  ...
  fillDonorSelect();
  const donorSel = document.getElementById("f_donor");
  const donorOptions = Array.from(donorSel.options).map((o) => o.value);
  if (e.donor && !donorOptions.includes(e.donor)) {
    const opt = document.createElement("option");
    opt.value = e.donor; opt.textContent = e.donor + " (list mein nahi)";
    donorSel.insertBefore(opt, donorSel.lastElementChild);
  }
  donorSel.value = e.donor || "";
  toggleDonorNewInput();
  ...
  document.querySelector('.side-nav-btn[data-tab="entry"]').click();
...
function renderRecords() {
  ...
  const dupCount = {};
  entries.forEach((e) => {
    if (!e.book || !e.receipt) return;
    const k = e.book + "|" + e.receipt;
  ...
  list.innerHTML = filtered.map((e) => {
    const isDup = e.book && e.receipt && dupCount[e.book + "|" + e.receipt] > 1;
    return `
    <div class="receipt-card ${isDup ? "dup-flag" : ""}">
      ${isDup ? '<span class="dup-flag-badge">⚠ Duplicate Book+Receipt — Admin check karein</span>' : ""}
      <div class="rc-top">
...
async function importFile(e) {
  ...
  try {
    ...
    const seen = new Set();
    const toInsert = [];
    const newDropdownVals = { accountType: new Set(), department: new Set(), subhead: new Set(), city: new Set(), donor: new Set() };
    ...
    mapped.forEach((r) => {
      ...
      seen.add(key);
      toInsert.push(r);
      ["accountType", "department", "subhead", "city", "donor"].forEach((k) => {
...
const DD_LABELS = { accountType: "Account Type", department: "Department", subhead: "Sub Head", city: "City", donor: "Donor Names" };
